...
const HomePage = () => {
    ...
    const getAllCategory = async () => {
        try {
        const { data } = await axios.get(`${process.env.SERVER_URL}/api/v1/category/get-category`);
        if (data?.success) {
    ...
    const getAllProducts = async () => {
        try {
        setLoading(true);
        const { data } = await axios.get(`${process.env.SERVER_URL}/api/v1/product/get-product`);
        setLoading(false);
    ...
    const getTotal = async () => {
        try {
        const { data } = await axios.get(process.env.SERVER_URL + "/api/v1/product/product-count");
        setTotal(data?.total);
        } catch (error) {
        console.log(error);
        }
    ...
    const loadMore = async () => {
        try {
        setLoading(true);
        const { data } = await axios.get(`${process.env.SERVER_URL}/api/v1/product/product-list/${page}`);
        setLoading(false);
    ...
    const filterProduct = async () => {
        try {
        const { data } = await axios.post(process.env.SERVER_URL+"/api/v1/product/product-filters", {
            checked,
            radio,
        });
        setProducts(data?.products);
